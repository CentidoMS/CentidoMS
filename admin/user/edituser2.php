<?php
	...
	if($user->checkperm("edituser")){
		...
		if($_POST["username"] == "" OR $_POST["group"] == "" OR $_POST["mail"] == ""){
			redirect("admin/index.php?page=user/users&id=".$_GET["id"]."&message=3");
			exit();
		}
		
		if($_POST["password"] != ""){
			if($_POST["password"] != $_POST["password2"]){
				redirect("admin/index.php?page=user/users&id=".$_GET["id"]."&message=4");
				exit();
			}else{
				$passwordcheck = true;
			}
		...
		if($count != 0){
			...
			if($id2 == $_GET["id"]){
				...
				redirect("admin/index.php?page=user/users&id=".$_GET["id"]."&message=5");
				exit();
			}
		}else{
			$usernamecheck = true;
		...
		redirect("admin/index.php?page=user/users&id=".$_GET["id"]."&message=1");
	}else{
		error(2, true);
	}
?>